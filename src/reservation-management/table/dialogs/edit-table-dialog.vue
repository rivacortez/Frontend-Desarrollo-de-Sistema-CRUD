...
const loadTable = async () => {
  ...
  loading.value = true;
  try {
    const data = await repository.getById(props.tableId);
    if (data) table.value = { ...data };
  } catch (err) {
    error.value = 'Error al cargar mesa';
    console.error(err);
  } finally {
    loading.value = false;
  }
...
const saveChanges = async () => {
  ...
  try {
    await repository.update(table.value.id, table.value);
    success.value = 'Mesa actualizada';
    emit('table-updated');
    setTimeout(() => {
